...
export class RedisService implements OnModuleInit, OnModuleDestroy {
  ...
  private setupEventListeners(): void {
    ...
    this.client.on('reconnecting', (delay) => {
      ...
      this.logger.log(`Redis reconnecting in ${delay}ms (attempt ${this.reconnectAttempts})`);
    });
  ...
  private async handleConnectionError(error: Error): Promise<void> {
    ...
    if (this.reconnectAttempts >= this.maxReconnectAttempts) {
      this.logger.error(`Redis connection failed after ${this.maxReconnectAttempts} attempts. Giving up.`);
      this.client = null;
      return;
    }

    const delay = Math.min(this.reconnectDelay * Math.pow(2, this.reconnectAttempts), 30000);
    this.logger.warn(`Redis connection failed: ${error.message}. Retrying in ${delay}ms...`);
  ...
  private async executeWithRetry<T>(
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (attempt > 1) {
          this.logger.log(`Redis ${operationName} succeeded on attempt ${attempt}`, { params });
        }
    ...
    if (operationName === 'exists' || operationName === 'set' || operationName === 'del' || operationName === 'expire' || operationName === 'flushdb') return false as T;
  ...
  async healthCheck(): Promise<{ isHealthy: boolean; error?: string; details?: any }> {
    if (!this.client) {
      return { 
        isHealthy: false, 
