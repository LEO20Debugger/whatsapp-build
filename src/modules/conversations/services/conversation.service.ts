...
export class ConversationService {
  ...
  private async handleBusinessLogic(
    ...
    try {
      ...
      if (
        ...
        await this.handleOrderCompletion(session, context);
      }
  ...
  /**
   * Handle order completion when payment is confirmed
  ...
  private async handleOrderCompletion(
    ...
    try {
      ...
      if (!orderId) {
        this.logger.warn(`No order ID found for completion: ${session.phoneNumber}`);
        return;
      }

      // Update order status to paid (this will be enhanced when payment service is implemented)
      await this.ordersService.updateOrderStatus(orderId, "paid", "Payment confirmed via WhatsApp");
      ...
      this.logger.log(
        `Order completed successfully for ${session.phoneNumber}: ${orderId}`,
      );
    } catch (error) {
      this.logger.error(
        `Failed to complete order for ${session.phoneNumber}: ${error.message}`,
        { error: error.stack },
      );
    }
  }
