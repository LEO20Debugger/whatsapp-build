...
export class InputParserService {
  ...
  async parseInput(
    ...
    try {
      ...
      const trigger = this.mapIntentToTrigger(intent, context.currentState as ConversationState);
  ...
  private detectIntent(input: string, context: ParsingContext): UserIntent {
    ...
    // Navigation patterns
    if (this.matchesPatterns(input, ['menu', 'show menu', 'view menu', 'what can i order'])) {
    ...
    if (this.matchesPatterns(input, ['start over', 'restart', 'begin again', 'new order'])) {
      return UserIntent.START_OVER;
    }

    if (this.matchesPatterns(input, ['help', 'how to', 'what do i do', 'commands'])) {
      return UserIntent.GET_HELP;
    }

    // Product patterns
    if (this.matchesPatterns(input, [
      'products', 'show products', 'what do you have', 'catalog', 'items'
    ])) {
    ...
    // Order patterns
    if (this.matchesPatterns(input, [
      'add', 'i want', 'order', 'buy', 'get me', 'add to cart'
    ])) {
      return UserIntent.ADD_TO_CART;
    }

    if (this.matchesPatterns(input, [
      'remove', 'delete', 'take out', 'remove from cart', 'dont want'
    ])) {
      return UserIntent.REMOVE_FROM_CART;
    }

    if (this.matchesPatterns(input, ['cart', 'my order', 'what do i have', 'show cart'])) {
      return UserIntent.VIEW_CART;
    }

    if (this.matchesPatterns(input, ['clear cart', 'empty cart', 'remove all'])) {
      return UserIntent.CLEAR_CART;
    }

    if (this.matchesPatterns(input, [
      'confirm', 'yes confirm', 'place order', 'proceed', 'checkout'
    ])) {
      return UserIntent.CONFIRM_ORDER;
    }

    if (this.matchesPatterns(input, ['cancel', 'cancel order', 'dont want', 'stop'])) {
      return UserIntent.CANCEL_ORDER;
    }

    // Payment patterns
    if (this.matchesPatterns(input, ['pay', 'payment', 'how to pay', 'payment details'])) {
      return UserIntent.MAKE_PAYMENT;
    }

    if (this.matchesPatterns(input, [
      'paid', 'payment done', 'sent payment', 'transferred', 'confirm payment'
    ])) {
      return UserIntent.CONFIRM_PAYMENT;
    }

    if (this.matchesPatterns(input, ['payment status', 'check payment', 'payment received'])) {
      return UserIntent.CHECK_PAYMENT_STATUS;
    }

    // General responses
    if (this.matchesPatterns(input, ['yes', 'yeah', 'yep', 'ok', 'okay', 'sure'])) {
  ...
  private extractEntities(input: string, intent: UserIntent): InputEntity[] {
    ...
    if (intent === UserIntent.ADD_TO_CART || intent === UserIntent.SEARCH_PRODUCT) {
      // Remove common words and extract potential product names
      const commonWords = ['i', 'want', 'to', 'order', 'buy', 'get', 'me', 'a', 'an', 'the', 'some'];
    ...
    const phoneMatches = input.match(/(\+?\d{1,3}[-.\s]?)?\(?\d{3}\)?[-.\s]?\d{3}[-.\s]?\d{4}/g);
    if (phoneMatches) {
    ...
    if (amountMatches && (intent === UserIntent.CONFIRM_PAYMENT || intent === UserIntent.MAKE_PAYMENT)) {
   ...
   */
  private getContextualYesTrigger(currentState: ConversationState): StateTrigger {
    switch (currentState) {
   ...
   */
  private getContextualNoTrigger(currentState: ConversationState): StateTrigger {
    switch (currentState) {
      case ConversationState.REVIEWING_ORDER:
   ...
   * Get entity by type
   */
  getEntityByType(entities: InputEntity[], type: EntityType): InputEntity | undefined {
