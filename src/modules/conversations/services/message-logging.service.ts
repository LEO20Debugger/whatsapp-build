...
export class MessageLoggingService {
  ...
  async updateMessageProcessingStatus(
    ...
    try {
      ...
      this.logger.debug(
        `Message processing status updated for ${phoneNumber}`,
        {
          messageId,
          isProcessed,
          errorMessage,
  ...
  async getConversationHistory(
    ...
    try {
      return await this.messageLogRepository.getConversationHistory(
        phoneNumber,
        limit
      );
    } catch (error) {
  ...
  private async getCustomerIdByPhoneNumber(
    ...
    try {
      const customer =
        await this.customersRepository.findByPhoneNumber(phoneNumber);
      return customer?.id;
    } catch (error) {
      this.logger.warn(
